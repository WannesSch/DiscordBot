...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        collector.on('collect', async (query) => {
            if (query.content.toLowerCase() === 'cancel') return message.reply(`Search cancelled ✅`) && collector.stop();

            const value = parseInt(query.content);

            if (!value || value <= 0 || value > maxTracks.length) return message.reply(`Invalid response, try a value between **1** and **${maxTracks.length}** or **cancel**... try again.`);
        ...
        collector.on('end', (msg, reason) => {
            if (reason === 'time') return message.reply(`Search timed out ${message.author}... try again.`);
        });
    },
};